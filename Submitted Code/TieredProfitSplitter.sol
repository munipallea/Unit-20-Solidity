...
contract TieredProfitSplitter {
    ...
    function deposit() public payable {
        uint points = msg.value / 100; //Divide into 100 units 
        uint total;
        uint amount;
        ...
        amount = points * 15; // Step 1: Set amount to equal `points` * the number of percentage points for this employee
        total += amount; // Step 2: Add the `amount` to `total` to keep a running total
        employee_three.transfer(amount); // Step 3: Transfer the `amount` to the employee
